...
当前稳定版：[**v2.6.7**](https://github.com/Aiyawoc/codex-luna-subagent-router/releases/tag/v2.6.7) · [更新记录](CHANGELOG.md) · [MIT License](LICENSE)

> **普通用户请下载 Release 中与系统和 CPU 匹配的完整包。** GitHub 自动生成的 `Source code.zip/.tar.gz` 不包含私有 Python 运行环境。v2.6.7 完整包内置 CPython 3.13.15，不依赖系统 Python、pip、uv 或 PATH。
...
| **三层模型** | `adaptive` 模式可在 Luna → Sol → Astra 中按任务需要升级能力；当前自动 Worker runtime IDs 为 `gpt-6-luna → gpt-6-sol → gpt-6-astra`。 |
...
```text
请安装或升级 Agent Router（Skill ID：`$codex-luna-subagent-router`）到当前稳定版 v2.6.7：
https://github.com/Aiyawoc/codex-luna-subagent-router/releases/tag/v2.6.7

先识别本机操作系统与 CPU 架构，下载匹配的 router-2.6.7-<platform> 完整包和校验文件。
...
从 [v2.6.7 Release](https://github.com/Aiyawoc/codex-luna-subagent-router/releases/tag/v2.6.7) 下载：
