...
class UpwardRoutingPolicyTests(unittest.TestCase):
    def test_root_skill_checks_capability_gap_before_lead_only(self) -> None:
        ...
        self.assertIn("在决定 `lead_only` 前", text)
        self.assertIn("Luna max", text)
        self.assertIn("不要先浪费一次低阶 attempt", text)
        self.assertIn("RoutePlan 2.1", text)

    def test_routing_policy_defines_capability_tiers_and_objective_signals(self) -> None:
    ...
    def test_eval_matrix_contains_explicit_low_to_high_routes(self) -> None:
        ...
        luna_scan = cases["luna-read-heavy-uproute"]
        self.assertEqual(luna_scan["expected_model"], "gpt-5.6-terra")
        self.assertEqual(luna_scan["expected_route_direction"], "up")

        terra_debug = cases["terra-hard-causal-uproute"]
        self.assertEqual(terra_debug["expected_model"], "gpt-5.6-sol")
        self.assertEqual(terra_debug["expected_route_direction"], "up")
