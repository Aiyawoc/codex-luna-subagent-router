...
5. 确定派遣后，使用 compact RoutePlan、fresh 新线程和最小充分任务包。派遣前简洁披露 task、model、effort 与成本理由；用户要求审批时才等待。
6. Worker 结果必须能用 `TASK_ACK <task_id>` 与当前目标核对；污染或目标不匹配时拒绝采纳。Lead 负责最终集成，并持续到用户请求的完成标准，而不是默认停在第一版实现。
