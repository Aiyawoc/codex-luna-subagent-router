...
Current stable release: [**v2.6.6**](https://github.com/Aiyawoc/codex-luna-subagent-router/releases/tag/v2.6.6) · [Changelog](CHANGELOG.md) · [MIT License](LICENSE)

> **End users should download the complete package matching their OS and CPU from the Release page.** GitHub's automatic `Source code.zip/.tar.gz` archives do not contain the private Python runtime. v2.6.6 complete packages include pinned CPython 3.13.15 and do not depend on system Python, pip, uv, or PATH.
...
```text
Install or upgrade Agent Router (Skill ID: `$codex-luna-subagent-router`) to stable v2.6.6:
https://github.com/Aiyawoc/codex-luna-subagent-router/releases/tag/v2.6.6

Detect the operating system and CPU architecture first, then download the matching
router-2.6.6-<platform> complete package and checksum files.
...
From the [v2.6.6 Release](https://github.com/Aiyawoc/codex-luna-subagent-router/releases/tag/v2.6.6), download:

| OS | CPU | Complete package |
|---|---|---|
| macOS | Apple Silicon / ARM64 | `router-2.6.6-macos-arm64.tar.gz` |
| macOS | Intel / x64 | `router-2.6.6-macos-x64.tar.gz` |
| Windows | x64 | `router-2.6.6-windows-x64.zip` |
| Windows | ARM64 | `router-2.6.6-windows-arm64.zip` |
